...
where
	...
	I: BlockImport<B, Error = ConsensusError, Transaction = sp_api::TransactionFor<C, B>>
		...
		+ Sync
		+ 'static,
	SO: SyncOracle<B> + Send + Sync + Clone + 'static,
...
impl<B: BlockT> BabeWorker<B> {
	...
	pub fn slot_notification_stream(
		...
		// log::info!("\nslot_notification_stream, client/babe/lib.rs");
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	fn claim_slot(
		...
		let mut i = 0;
		let (timeout_count, interval) = (10, 100);
		loop{
			if let Ok(Some(ret)) = rx.try_next(){
				vote_result.push(ret);
				// if vote_result.len() == author_count{
				// 	break;
				// }
			}
		...
		let mut claim_success = false;
		if vote_result[0] == local_random{
		...
		if !claim_success{
			log::info!("\nlocal: {}, all: {:?}, claim_slot(), babe/lib.rs", local_random, vote_result);
			return None;
		}
		else{
			log::info!("\nlocal: {}, all: {:?}, claim_slot(), babe/lib.rs, ------claim slot", local_random, vote_result);
		}
		...
		// 	){
		// 		log::info!("\nlocal index: {}, claim_slot(), babe/lib.rs", authority_index);
		// 		// return Some(PreDigest::SecondaryPlain(SecondaryPlainPreDigest{
		// 		// 	slot,
	...
	fn notify_slot(
		...
		// log::info!("\nslot_len: {}, client/babe/lib.rs", self.slot_notification_sinks.lock().len());
		...
		// let keys = SyncCryptoStore::keys(&*self.keystore, sp_application_crypto::key_types::BABE);
		// for (index, key) in keys.iter().enumerate(){
		// 	log::info!("\n{}: {:?}, notify_slot, babe/lib.rs", index, key);
		// }
	}
