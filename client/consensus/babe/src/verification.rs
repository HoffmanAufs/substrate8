	...
	DigestItemFor<B>: CompatibleDigestItem,
{
	...
	// let author_index = pre_digest.authority_index();
	// log::info!("\nauthor index: {} babe verification.rs", author_index);
	
	let author = match authorities.get(pre_digest.authority_index() as usize) { Some(author) => author.0.clone(),
