...
pub fn claim_slot(
	...
	// let local_keys = SyncCryptoStore::sr25519_public_keys(&**keystore, AuthorityId::ID);
		
	// if local_keys.len()>0{
	for (authority_id, authority_index) in authorities.iter(){
		if SyncCryptoStore::has_keys(
			&**keystore,
			&[(authority_id.to_raw_vec(), AuthorityId::ID)]
		){
			let pre_digest = PreDigest::SecondaryPlain(
	...
	// 	log::info!("\n{}, {:?}, claim_slot, authoriship.rs", i.0, i.1);
	// }

	// let result = claim_slot_using_keys(slot, epoch, keystore, &authorities);
	// match &result{
	// 	Some((PreDigest::Primary(_), _)) =>{
	// 		log::info!("PreDigest::Primary, claim_slot(), authorship.rs")
	// 	},
	// 	Some((PreDigest::SecondaryPlain(_), _)) =>{
	// 		log::info!("PreDigest::SecondaryPlain, claim_slot(), authorship.rs")
	// 	},
	// 	Some((PreDigest::SecondaryVRF(_), ))) =>{
	// 		log::info!("PreDigest::SecondaryVRF, claim_slot(), authorship.rs")
	// 	},
	// 	None=>{
	// 		log::info!("PreDigest::None, claim_slot(), authoriship.rs")
	// 	}
	// }
	// return result;
}
...
pub fn claim_slot_using_keys(
	...
	// log::info!("\n{}, claim_slot_using_keys(), authorship.rs", epoch.config.allowed_slots.is_secondary_vrf_slots_allowed());
...
fn claim_primary_slot(
	...
	for (authority_id, authority_index) in keys {
		// log::info!("\n{:?}, {}, claim_primary_slot, authoriship.rs" ,authority_id, authority_index);
