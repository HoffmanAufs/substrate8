	...
	IDP::InherentDataProviders: crate::InherentDataProviderExt,
{
	pub async fn next_block(&mut self) -> Result<SlotInfo<Block>, Error>{
		loop{
			...
			self.best_hash = match self.best_hash{
				None=>{
					log::info!("new block");
					Some(best_hash)
				},
				Some(last_hash)=>{
					if last_hash == best_hash{
						log::info!("Same last_hash, wait next block, next_block()");
						// Some(best_hash)
						continue;
					}
					else{
						log::info!("Update last_hash, next_block()");
						Some(best_hash)
					}
				},
			};
	...
	pub async fn next_slot(&mut self) -> Result<SlotInfo<Block>, Error> {
		loop {
			...
			// let best_hash = chain_head.hash();
			// self.best_hash = match self.best_hash.take(){
			// 	None=>{
			// 		Some(best_hash)
			// 	},
			// 	Some(last_hash)=>{
			// 		if last_hash == best_hash{
			// 			log::info!("Same last_hash, wait next block, next_slot()");
			// 			Some(best_hash)
			// 			// continue;
			// 		}
			// 		else{
			// 			log::info!("Update last_hash, next_slot()");
			// 			Some(best_hash)
			// 		}
			// 	},
			// };
