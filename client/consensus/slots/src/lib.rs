...
pub trait SimpleSlotWorker<B: BlockT> {
	...
	async fn on_slot(
		...
		// log::info!("consensus on_slot: {:?}", slot);
		...
		// log::info!("{:?}, {:?}", proposing_remaining_duration, default_duration);
		...
		// aura function
		// self.aura_custom(&slot_info.chain_head, slot, &epoch_data);
		...
		// 	vote_result.push(random_number);
		// 	self.sync_oracle().send_number(random_number, tx);
		// }

		// let mut timer = Delay::new(Duration::from_millis(2000)).fuse();
		// loop{
		// 	select!{
		// 		_ = timer =>{
		// 			log::info!("times up");
		...
		// 		vote_from_other = rx.select_next_some().fuse()=>{
		// 			log::info!("recevie number: {}", vote_from_other);
		// 			vote_result.push(vote_from_other);
		// 		}
		// 	}
		// }
		...
		// let arua_result = self.aura_claim(&slot_info.chain_head, slot, &epoch_data);
		// log::info!("aura_result: {}", arua_result);
		// if self.aura_claim(&slot_info.chain_head, slot, &epoch_data)==false{
		// 	return None;
		// }
		...
		// log::info!("\nReturn SlotResult, /consensus/slot/lib.rs");
	...
	CAW: CanAuthorWith<B> + Send,
{
	...
	loop {
			...
			can_author_with.can_author_with(&BlockId::Hash(slot_info.chain_head.hash()))
		{
			...
			log::info!("on_slot: {}", slot_info.slot);
