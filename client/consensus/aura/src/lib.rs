...
pub use sc_consensus_aura_slots::SlotProportion;
...
pub type SlotDuration = sc_consensus_aura_slots::SlotDuration<sp_consensus_aura::SlotDuration>;
...
impl<B, C, E, I, P, Error, SO, L, BS> sc_consensus_aura_slots::SimpleSlotWorker<B>
	...
	Error: std::error::Error + Send + From<sp_consensus::Error> + 'static,
{
	...
	fn pre_digest_data(
		...
		vec![<DigestItemFor<B> as CompatibleDigestItem<P::Signature>>::aura_pre_digest(claim.0.clone())]
	}
	...
	fn proposing_remaining_duration(&self, slot_info: &SlotInfo<B>) -> std::time::Duration {
		...
		sc_consensus_aura_slots::proposing_remaining_duration(
			parent_slot,
			slot_info,
			&self.block_proposal_slot_portion,
			self.max_block_proposal_slot_portion.as_ref(),
			sc_consensus_aura_slots::SlotLenienceType::Exponential,
			self.logging_target(),
		)
	}
	...
	fn verify_election(&mut self, election_data: &ElectionData<B>, &cur_hash: &B::Hash)->bool{
		...
		// check signature
		let ElectionData{hash, sig_bytes, ranks, pub_bytes} = election_data;
		if let Ok(sig) = <P::Signature as Decode>::decode(&mut sig_bytes.as_slice()){
			let mut msg_bytes :Vec<u8> = vec![];
			msg_bytes.extend(hash.encode().iter());
			msg_bytes.extend(ranks.encode().iter());
		...
		if sr25519_public_keys.len() == 1{
			...
			if let Ok(committee) = authorities(self.client.as_ref(), &BlockId::Hash(cur_hash.clone())){
				...
				for election in election_vec.iter(){
					if let Some(cur_rank) = election.iter().position(|bytes|&pub_bytes == bytes){
...
fn caculate_min_max_value(committee_count: usize, max_vote_rank: usize)->(u64, u64){
	let max = {
		let mut ret = 1;
