...
impl RequestResponsesBehaviour {
	...
	pub fn send_request(
		...
		// if protocol_name.ne("/sup/sync/2"){
		// 	println!("send request: {}", protocol_name);
		// }
		// log::info!("\n0x{:02x}{:02x}({}) [{}]---request_response.rs send_request()", request[8], request[9], request.len(), target);
		if let Some((protocol, _)) = self.protocols.get_mut(protocol_name) {
			if protocol.is_connected(target) || connect.should_connect() {
				let request_id = protocol.send_request(target, request);
				// log::info!("\n{}({}) send request----request_response.rs send_request()", protocol_name, request_id);
...
impl NetworkBehaviour for RequestResponsesBehaviour {
	...
	fn inject_event(
		...
		// log::info!("inject event: {:?}, {:?}", p_name, event);
		// if p_name.ne("/sup/sync/2"){
		// 	log::info!("request response: inject event{}", p_name);
		// }
		if let Some((proto, _)) = self.protocols.get_mut(&*p_name) {
			// if let libp2p_request_response::handler::RequestResponseHandlerEvent::<sc_network::request_response::GenericCodec>::Request{_, request, _} = event{
			// 	log::info!("{}, {}", request[8], request[9]);
			// }
			// if let RequestResponseHandlerEvent::<GenericCodec> = event{
			// 	log::info!("1");
			// }
		...
		'poll_all: loop {
			...
			while let Poll::Ready(Some(outcome)) = self.pending_responses.poll_next_unpin(cx) {
				...
				if let Ok(payload) = result {
					if let Some((protocol, _)) = self.protocols.get_mut(&*protocol_name) {
						// log::info!("\n0x{:02x}{:02x}({}) [{}]---send response, data from pending_response, poll() request_response.rs", payload[8], payload[9], payload.len(), peer);
						if let Err(_) = protocol.send_response(inner_channel, Ok(payload)) {
							...
							// log::info!("\n{}({}) send response----request_response.rs poll()", protocol_name, request_id);
				...
				while let Poll::Ready(ev) = behaviour.poll(cx, params) {
					...
					let ev = match ev {
						...
						NetworkBehaviourAction::GenerateEvent(ev) => {
							// log::info!("generate_event, {}", protocol);
							ev
						},
					...
					match ev {
						...
						RequestResponseEvent::Message {
							...
							// log::info!("\n{}({}) receive request----request_response.rs poll()", protocol, request_id);
							...
							if let Some(resp_builder) = resp_builder {
								...
								// log::info!("\n0x{:02x}{:02x}({}) [{}]---request_response.rs from inject_event()", request[8], request[9], request.len(), peer);
						...
						RequestResponseEvent::Message {
							...
							// 	log::info!("\n0x{:02x}{:02x}({}) [{}]---receive response poll() request_response.rs", data[8], data[9], data.len(), peer);
							// }
