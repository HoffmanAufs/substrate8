...
fn prepare_block_request<B: BlockT>(
	...
	// if let Ok(_) = request.encode(&mut buf){
	// 	log::info!("\n0x{:02x}{:02x}({}) [{}] ---protocol.rs fn prepare_block_request() ", buf[8], buf[9], buf.len(), who);
	// }
...
impl<B: BlockT> NetworkBehaviour for Protocol<B> {
	...
	>{
					...
					Poll::Ready(Ok(Ok(resp))) => {
						...
						match req {
							PeerRequest::Block(req) => {
								// messge send from request_response.rs fn poll() protocol.send_response()
								// log::info!("\n0x{:02x}{:02x}({}) BlockResponse poll() protocol.rs", resp[8], resp[9], resp.len());
		...
		for (id, req, protobuf_response) in finished_block_requests {
			// log::info!("finished block_request: {:?}", id);
		...
		for (id, request) in self.sync.block_requests() {
			// log::info!("prepare block request: \n{:?}", request);
			...
			NotificationsOut::Notification { peer_id, set_id, message } => match set_id {
				...
				_ => {
					let protocol_name = self.notification_protocols
						[usize::from(set_id) - NUM_HARDCODED_PEERSETS]
						.clone();
					// if protocol_name.ne("/paritytech/grandpa/1"){
					// 	log::info!("NotificationsOut::Notification {} (protocol.rs)", protocol_name);
					// }
