...
use crate::{
	...
	bitswap::Bitswap,
	config::{parse_str_addr, Params, TransportConfig, Role},
...
pub struct NetworkService<B: BlockT + 'static, H: ExHashT> {
	...
	/// Local copy of the role of the local node.
	local_role: Role,
...
impl<B: BlockT + 'static, H: ExHashT> NetworkWorker<B, H> {
	...
	pub fn new(mut params: Params<B, H>) -> Result<NetworkWorker<B, H>, Error> {
		...
		let producer_select_handler_proto =
			producer_select::ProducerSelectHandlerPrototype::new(params.protocol_id.clone());
		params
			.network_config
			.extra_sets
			.insert(0, producer_select_handler_proto.set_config());
		...
		let local_role = params.role.clone();
		// log::info!("Role: {:?}", params.role);
		...
		let (ps_handler, ps_handler_controller) = producer_select_handler_proto.build(
			service.clone(),
			params.metrics_registry.as_ref(),
		)?;
		(params.producer_select_handler_executor)(ps_handler.run().boxed());
...
impl<B: BlockT + 'static, H: ExHashT> NetworkService<B, H> {
	...
	pub fn notification_sender(
		...
		if protocol.ne("/paritytech/grandpa/1"){
			log::info!("service.rs fn notification_sender(){}", protocol);
		}
		let sink = {
			let peers_notifications_sinks = self.peers_notifications_sinks.lock();
...
pub struct NetworkWorker<B: BlockT + 'static, H: ExHashT> {
	...
	ps_handler_controller: producer_select::ProducerSelectHandlerController<B>,
}
				...
				Poll::Ready(SwarmEvent::Behaviour(BehaviourOut::NotificationsReceived {
					...
					if let Some(metrics) = this.metrics.as_ref() {
						for (protocol, message) in &messages {
							// if protocol.ne("/paritytech/grandpa/1"){
							// 	log::info!("poll NotificationReceived: {}", protocol);
							// }
							metrics
								.notifications_sizes
